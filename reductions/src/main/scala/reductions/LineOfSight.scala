  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    tree match {
      ...
      case Node(left, right) =>{
        val maxLeft = if(startingAngle > left.maxPrevious) startingAngle else left.maxPrevious
        parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, maxLeft, right))
      }
    }
  }
